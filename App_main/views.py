...
def home(request):
    ...
    profile = ProfileModel.objects.filter(user=request.user)
    if profile.exists():
        myProfile = True
    else:
        myProfile = False
    medicines = MedicineModel.objects.all()[:100]
    covid_ = []
    women_ = []
...
def all_medicine(request):
    medicines = MedicineModel.objects.all()[:200]
...
def remove_item_from_cart(request, itemID):
    product = MedicineModel.objects.get(id=itemID)
    cart_item = CartModel.objects.get(user=request.user, medicineItem=product, ordered=False)
    cart_item.delete()
    return HttpResponseRedirect(reverse('App_main:home'))
